/**
 * Mock Palworld dedicated server for local testing.
 * Implements a Source-style RCON protocol compatible with rcon-client,
 * and writes fake SaveGames / logs / config under a data directory.
 *
 * Usage:
 *   npx tsx scripts/mock-palworld-server.ts --data ./simulator-data --rcon-port 25575
 */
import net from 'net'
import fs from 'fs'
...
function ensureLayout(): void {
  ...
  if (!fs.existsSync(saveMarker)) {
    fs.writeFileSync(saveMarker, 'MOCK_SAVE_DATA', 'utf8')
  }

  const modPath = path.join(DATA_DIR, 'Pal', 'Content', 'Paks', '~mods', 'ExampleQoL.pak')
  if (!fs.existsSync(modPath)) {
    fs.writeFileSync(modPath, Buffer.alloc(128, 1))
  }
  const disabled = path.join(DATA_DIR, 'Pal', 'Content', 'Paks', '~mods', 'OldMod.pak.disabled')
  if (!fs.existsSync(disabled)) {
    fs.writeFileSync(disabled, Buffer.alloc(64, 2))
  }
...
function pack(id: number, type: number, body: string): Buffer {
...
function handleCommand(cmd: string): string {
  ...
  if (lower.startsWith('broadcast ')) {
    const msg = trimmed.slice('Broadcast '.length)
    appendLog(`[INFO] Broadcast: ${msg}`)
  ...
  if (lower === 'info') {
    return 'Welcome to Palworld / Simulated dedicated server v0.0.1'
  }
  ...
  socket.on('data', (chunk) => {
    ...
    while (buffer.length >= 4) {
      ...
      if (type === SERVERDATA_EXECCOMMAND) {
        ...
        socket.write(pack(id, SERVERDATA_RESPONSE_VALUE, response))
      }
    }
  })
